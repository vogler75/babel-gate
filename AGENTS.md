...
Other CLI flags (`cmd/router/main.go`): `-port`, `-copilot-login` (RFC 8628 device flow, writes to `copilot.GetTokenFilePath()`), `-background`/`-d` (daemon, no TUI), `-no-tui`, `-tray` (Windows notification-area icon; implies `-background`), `-log-file`, `-log-max-size-mb`, `-log-max-backups`.
