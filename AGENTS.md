...
Other CLI flags (`cmd/router/main.go`): `-port`, `-copilot-login` (RFC 8628 device flow, writes to `copilot.GetTokenFilePath()`), `-background`/`-d` (daemon, no TUI), `-no-tui`, `-log-file`, `-log-max-size-mb`, `-log-max-backups`.
