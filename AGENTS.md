...
Other CLI flags (`cmd/router/main.go`): `-port`, `-copilot-login` (RFC 8628 device flow, writes to `copilot.GetTokenFilePath()`), `-background`/`-d` (no TUI, logs to file; on Windows it additionally detaches and shows a notification-area icon), `-no-tui`, `-log-file`, `-log-max-size-mb`, `-log-max-backups`.
...
- **`pkg/tui`** — the default foreground UI, a hand-rolled ANSI renderer with CJK-aware width handling; keys: `q`/Ctrl-C quit, `j/k`+arrows/PgUp/PgDn scroll, `h/l`/`0` horizontal scroll, `c` clear, `r` re-measure. Disabled by `-no-tui`/`-background`, a non-TTY, or `HostSupportsTUI()` (`host_{windows,other}.go`) reporting a console host that cannot render ANSI — on Windows that check walks a `Toolhelp32` snapshot to the parent process and allowlists PowerShell-class hosts, since `PSModulePath` is machine-wide and cannot distinguish cmd.exe from PowerShell. `pkg/logger` provides the size-based rotating file writer plus a 1000-line `RingBuffer`, combined via `MultiWriterWithRing` so the TUI and log file share one `log.SetOutput`. `sigwinch_{posix,windows}.go` are build-tagged.

- **`pkg/tray`** — notification-area icon for background instances. There is no separate flag: `main.go` computes `trayMode := *background && tray.Supported()`, so `-background` gains the tray (and detaching) exactly where a tray exists. Windows-only: `tray_windows.go` is pure `syscall` against `user32.dll`/`shell32.dll` (hidden top-level window + `Shell_NotifyIconW` + `TrackPopupMenu`), so `CGO_ENABLED=0` cross-builds keep working — **never** reach for a systray library or cgo here. `tray_other.go` makes `Supported()` false everywhere else, so `-background` degrades to plain foreground background mode. Pointers handed to `LazyProc.Call` are held in named locals with `runtime.KeepAlive`, since the `unsafe.Pointer`→`uintptr` GC exemption does not cover variadic `Call` arguments. Icon selection is split into the pure `ico.go` (`pickIconEntry`) so it is testable off-Windows; `icon.ico` is generated art, not hand-drawn. The menu is deliberately just Open Dashboard + Quit.

- **`pkg/daemon`** — `Detach()` re-execs the current binary with the same args and cwd, stdio pointed at `os.DevNull`, and marks the child with the `BABELGATE_DAEMON=1` env var that `IsChild()` reads to prevent a respawn loop; `sysproc_{windows,other}.go` supply `DETACHED_PROCESS|CREATE_NEW_PROCESS_GROUP` and `Setsid` respectively. Only the tray path uses it — **`-background` on non-Windows deliberately stays in the foreground** so Docker/systemd supervision keeps working. `main.go` calls it *after* config load and rotator init so those failures still reach the console, and everything after it is log-file-only.
